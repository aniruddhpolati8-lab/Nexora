
NEXORA v10
Intelligence. Secured.

Local-first AI assistant.
No external language model required.

Features:
- Local reasoning engine
- Intent classification
- Conversation context
- Persistent memory
- Knowledge base
- Relevance scoring
- Safe calculator
- API-key authentication
- Rate limiting
- Secret detection
- Security event logging
- Security dashboard
- Lockdown / emergency state
- Atomic persistence
- Thread-safe state
- Safe HTTP server
============================================================
"""

from __future__ import annotations

import ast
import hashlib
import hmac
import json
import math
import os
import re
import secrets
import threading
import time
from collections import Counter, defaultdict, deque
from datetime import datetime, timezone
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer
from urllib.parse import urlparse


# ============================================================
# CONFIGURATION
# ============================================================

APP_NAME = "Nexora"
VERSION = "10.0"
SLOGAN = "Intelligence. Secured."

HOST = os.environ.get("HOST", "0.0.0.0")
PORT = int(os.environ.get("PORT", "8000"))

DATA_FILE = os.environ.get(
    "NEXORA_DATA_FILE",
    "nexora_data.json"
)

MAX_INPUT = 5000
MAX_OUTPUT = 10000

MAX_MEMORY = 800
MAX_MEMORIES = 500

MAX_HISTORY = 120

MAX_KNOWLEDGE_KEY = 200
MAX_KNOWLEDGE_VALUE = 5000

RATE_WINDOW = 60
RATE_LIMIT = 60

MAX_REQUEST_BYTES = 100_000

# API authentication.
#
# DO NOT put your real key in this file.
#
# Render:
#
# NEXORA_API_KEY = your-secret-key
#
API_KEY = os.environ.get("NEXORA_API_KEY", "").strip()


# ============================================================
# GLOBAL STATE
# ============================================================

state_lock = threading.RLock()

security_state = "normal"

memories: list[str] = []

knowledge: dict[str, str] = {}

conversation = deque(maxlen=MAX_HISTORY)

request_history = defaultdict(deque)

security_events = deque(maxlen=500)

metrics = {
    "requests": 0,
    "successful_requests": 0,
    "failed_requests": 0,
    "blocked_requests": 0,
    "authenticated_requests": 0,
    "started_at": time.time(),
}

settings = {
    "mode": "friendly",
    "response_length": "normal",
    "emoji": True,
    "user_name": None,
}


# ============================================================
# EXCEPTIONS
# ============================================================

class SafetyFailure(Exception):
    """Safe, expected failure."""
    pass


class AuthenticationFailure(Exception):
    """Authentication failed."""
    pass


# ============================================================
# SECURITY EVENTS
# ============================================================

def security_event(name: str, severity: str = "INFO"):
    event = {
        "event": name,
        "severity": severity,
        "timestamp": time.time(),
    }

    with state_lock:
        security_events.append(event)


def is_locked() -> bool:
    with state_lock:
        return security_state in (
            "lockdown",
            "emergency",
        )


def enter_lockdown():
    global security_state

    with state_lock:
        security_state = "lockdown"

    security_event(
        "lockdown_enabled",
        "CRITICAL",
    )


def emergency_stop():
    global security_state

    with state_lock:
        security_state = "emergency"

    security_event(
        "emergency_stop",
        "CRITICAL",
    )


def reset_security():
    global security_state

    with state_lock:
        security_state = "normal"

    security_event(
        "security_state_reset",
        "WARN",
    )


# ============================================================
# API KEY SECURITY
# ============================================================

def generate_api_key() -> str:
    """
    Generates a strong random API key.

    This is useful for creating a key locally.

    Example:
        python nexora.py --generate-key
    """
    return secrets.token_urlsafe(48)


def hash_api_key(key: str) -> str:
    return hashlib.sha256(
        key.encode("utf-8")
    ).hexdigest()


def authenticate_request(handler: BaseHTTPRequestHandler):
    """
    Requires:

        Authorization: Bearer YOUR_KEY
    """

    if not API_KEY:
        security_event(
            "api_key_not_configured",
            "CRITICAL",
        )
        raise AuthenticationFailure(
            "API key is not configured."
        )

    header = handler.headers.get(
        "Authorization",
        "",
    ).strip()

    expected = "Bearer " + API_KEY

    if not hmac.compare_digest(
        header,
        expected,
    ):
        security_event(
            "authentication_failed",
            "WARN",
        )
        raise AuthenticationFailure()

    with state_lock:
        metrics["authenticated_requests"] += 1


# ============================================================
# SECRET DETECTION
# ============================================================

SECRET_PATTERNS = [
    r"sk-[A-Za-z0-9_-]{20,}",
    r"AIza[A-Za-z0-9_-]{20,}",
    r"(?i)\bpassword\s*[:=]\s*\S+",
    r"(?i)\bapi[_-]?key\s*[:=]\s*\S+",
    r"(?i)\bsecret\s*[:=]\s*\S+",
    r"(?i)\btoken\s*[:=]\s*\S+",
    r"(?i)\bbearer\s+[A-Za-z0-9._-]{20,}",
    r"ghp_[A-Za-z0-9]{20,}",
    r"github_pat_[A-Za-z0-9_]{20,}",
]


def contains_secret(text: str) -> bool:
    if not isinstance(text, str):
        return False

    return any(
        re.search(pattern, text)
        for pattern in SECRET_PATTERNS
    )


# ============================================================
# SAFETY ENGINE
# ============================================================

SELF_HARM_PATTERNS = [
    r"\bkill myself\b",
    r"\bend my life\b",
    r"\bhurt myself\b",
    r"\bself[- ]?harm\b",
    r"\bsuicide\b",
]


DANGEROUS_PATTERNS = [
    r"\bhow\s+to\s+kill\s+someone\b",
    r"\bhow\s+to\s+hurt\s+someone\b",
    r"\bhow\s+to\s+poison\s+someone\b",
    r"\bhow\s+to\s+make\s+a\s+bomb\b",
    r"\bhow\s+to\s+build\s+a\s+bomb\b",
    r"\bhow\s+to\s+make\s+an\s+explosive\b",
    r"\bhow\s+to\s+build\s+an\s+explosive\b",
    r"\bhow\s+to\s+make\s+a\s+weapon\b",
    r"\bhow\s+to\s+build\s+a\s+weapon\b",
]


RISKY_PATTERNS = [
    r"\bdeadly\s+challenge\b",
    r"\bdangerous\s+challenge\b",
    r"\bchoking\s+challenge\b",
    r"\bhow\s+to\s+get\s+high\b",
    r"\bhow\s+to\s+starve\b",
    r"\bhow\s+to\s+purge\b",
    r"\bexercise\s+until\s+i\s+collapse\b",
]


def safety_check(text: str):
    lower = text.lower()

    for pattern in SELF_HARM_PATTERNS:
        if re.search(pattern, lower):
            return False, "self_harm"

    for pattern in DANGEROUS_PATTERNS:
        if re.search(pattern, lower):
            return False, "dangerous"

    for pattern in RISKY_PATTERNS:
        if re.search(pattern, lower):
            return False, "risky"

    return True, "safe"


def safety_response(category: str) -> str:
    if category == "self_harm":
        return (
            "I can't provide instructions for hurting "
            "yourself. Please talk to a trusted adult or "
            "someone who can support you."
        )

    if category == "dangerous":
        return (
            "I can't provide instructions for seriously "
            "harming people or creating dangerous weapons."
        )

    if category == "risky":
        return (
            "I can't encourage dangerous habits or "
            "challenges."
        )

    return "I can't safely help with that."


# ============================================================
# INPUT VALIDATION
# ============================================================

def validate_input(message):
    if not isinstance(message, str):
        raise SafetyFailure()

    message = message.strip()

    if not message:
        raise SafetyFailure()

    if len(message) > MAX_INPUT:
        security_event(
            "oversized_input",
            "WARN",
        )
        raise SafetyFailure()

    return message


# ============================================================
# RATE LIMITING
# ============================================================

def rate_limit(client_id: str):
    now = time.time()

    with state_lock:
        history = request_history[client_id]

        while (
            history
            and now - history[0] > RATE_WINDOW
        ):
            history.popleft()

        if len(history) >= RATE_LIMIT:
            security_event(
                "rate_limit_triggered",
                "WARN",
            )
            raise SafetyFailure()

        history.append(now)


# ============================================================
# PERSISTENT STORAGE
# ============================================================

def load_data():
    global memories
    global knowledge
    global settings

    if not os.path.exists(DATA_FILE):
        return

    try:
        with open(
            DATA_FILE,
            "r",
            encoding="utf-8",
        ) as file:
            data = json.load(file)

        loaded_memories = data.get(
            "memories",
            [],
        )

        if isinstance(
            loaded_memories,
            list,
        ):
            memories = [
                item
                for item in loaded_memories
                if isinstance(item, str)
            ][:MAX_MEMORIES]

        loaded_knowledge = data.get(
            "knowledge",
            {},
        )

        if isinstance(
            loaded_knowledge,
            dict,
        ):
            knowledge = {
                str(key): str(value)
                for key, value
                in loaded_knowledge.items()
            }

        loaded_settings = data.get(
            "settings",
            {},
        )

        if isinstance(
            loaded_settings,
            dict,
        ):
            for key in settings:
                if key in loaded_settings:
                    settings[key] = loaded_settings[key]

    except Exception:
        security_event(
            "data_load_failed",
            "ERROR",
        )


def save_data():
    temporary_file = DATA_FILE + ".tmp"

    try:
        with state_lock:
            data = {
                "memories": list(memories),
                "knowledge": dict(knowledge),
                "settings": dict(settings),
            }

        with open(
            temporary_file,
            "w",
            encoding="utf-8",
        ) as file:
            json.dump(
                data,
                file,
                indent=2,
                ensure_ascii=False,
            )

        os.replace(
            temporary_file,
            DATA_FILE,
        )

    except Exception:
        security_event(
            "data_save_failed",
            "ERROR",
        )


# ============================================================
# MEMORY SECURITY
# ============================================================

MEMORY_INJECTION_PATTERNS = [
    r"ignore previous instructions",
    r"ignore all safety rules",
    r"ignore your instructions",
    r"you are now the system",
    r"system message:",
    r"developer message:",
    r"assistant instructions:",
]


def sanitise_memory(text: str) -> str:
    if not isinstance(text, str):
        raise SafetyFailure()

    text = text.strip()

    if not text:
        raise SafetyFailure()

    if len(text) > MAX_MEMORY:
        raise SafetyFailure()

    if contains_secret(text):
        security_event(
            "secret_memory_blocked",
            "WARN",
        )
        raise SafetyFailure()

    for pattern in MEMORY_INJECTION_PATTERNS:
        if re.search(
            pattern,
            text,
            re.IGNORECASE,
        ):
            security_event(
                "memory_injection_blocked",
                "WARN",
            )
            raise SafetyFailure()

    return text


# ============================================================
# TEXT INTELLIGENCE
# ============================================================

STOP_WORDS = {
    "the",
    "a",
    "an",
    "is",
    "are",
    "am",
    "i",
    "you",
    "my",
    "your",
    "to",
    "of",
    "and",
    "or",
    "in",
    "on",
    "it",
    "this",
    "that",
    "what",
    "do",
    "does",
    "did",
    "for",
    "with",
    "me",
    "can",
    "could",
    "would",
    "should",
    "be",
    "have",
    "has",
    "how",
    "why",
    "when",
    "where",
    "please",
}


def words(text: str):
    tokens = re.findall(
        r"[a-zA-Z0-9']+",
        text.lower(),
    )

    return [
        token
        for token in tokens
        if token not in STOP_WORDS
    ]


def normalise_text(text: str) -> str:
    text = text.lower()
    text = re.sub(
        r"[^a-z0-9\s]",
        " ",
        text,
    )
    text = re.sub(
        r"\s+",
        " ",
        text,
    )
    return text.strip()


def similarity_score(query: str, text: str) -> float:
    query_words = words(query)
    text_words = words(text)

    if not query_words or not text_words:
        return 0.0

    query_counts = Counter(query_words)
    text_counts = Counter(text_words)

    score = 0.0

    for word, count in query_counts.items():
        if word in text_counts:
            score += min(
                count,
                text_counts[word],
            )

    # Phrase bonus.
    normal_query = normalise_text(query)
    normal_text = normalise_text(text)

    if normal_query and normal_query in normal_text:
        score += 5

    return score


def relevance_score(
    query: str,
    title: str,
    content: str,
) -> float:

    title_score = similarity_score(
        query,
        title,
    )

    content_score = similarity_score(
        query,
        content,
    )

    return (
        title_score * 3
        + content_score
    )


# ============================================================
# MEMORY ENGINE
# ============================================================

class MemoryEngine:

    @staticmethod
    def save(text: str):
        text = sanitise_memory(text)

        with state_lock:
            if text not in memories:
                memories.append(text)

            while len(memories) > MAX_MEMORIES:
                memories.pop(0)

        save_data()

    @staticmethod
    def all():
        with state_lock:
            return list(memories)

    @staticmethod
    def clear():
        with state_lock:
            memories.clear()

        save_data()

    @staticmethod
    def search(
        query: str,
        limit: int = 8,
    ):

        scored = []

        for memory in MemoryEngine.all():

            score = similarity_score(
                query,
                memory,
            )

            if score > 0:
                scored.append(
                    (score, memory)
                )

        scored.sort(
            key=lambda item: item[0],
            reverse=True,
        )

        return [
            memory
            for _, memory
            in scored[:limit]
        ]

    @staticmethod
    def delete_matching(
        query: str,
    ):

        results = MemoryEngine.search(
            query,
            limit=1,
        )

        if not results:
            return False

        target = results[0]

        with state_lock:
            if target in memories:
                memories.remove(target)
            else:
                return False

        save_data()

        return True


# ============================================================
# KNOWLEDGE ENGINE
# ============================================================

class KnowledgeEngine:

    @staticmethod
    def add(
        key: str,
        value: str,
    ):

        key = key.strip()
        value = value.strip()

        if not key or not value:
            return False

        if len(key) > MAX_KNOWLEDGE_KEY:
            return False

        if len(value) > MAX_KNOWLEDGE_VALUE:
            return False

        if contains_secret(key):
            return False

        if contains_secret(value):
            return False

        with state_lock:
            knowledge[key.lower()] = value

        save_data()

        return True

    @staticmethod
    def search(
        query: str,
        limit: int = 5,
    ):

        results = []

        with state_lock:
            items = list(
                knowledge.items()
            )

        for key, value in items:

            score = relevance_score(
                query,
                key,
                value,
            )

            if score > 0:
                results.append(
                    (score, key, value)
                )

        results.sort(
            key=lambda item: item[0],
            reverse=True,
        )

        return results[:limit]


# ============================================================
# CONTEXT ENGINE
# ============================================================

class ContextEngine:

    @staticmethod
    def add(
        role: str,
        content: str,
    ):

        with state_lock:
            conversation.append({
                "role": role,
                "content": content,
                "timestamp": datetime.now(
                    timezone.utc
                ).isoformat(),
            })

    @staticmethod
    def recent(limit: int = 12):

        with state_lock:
            return list(
                conversation
            )[-limit:]

    @staticmethod
    def clear():

        with state_lock:
            conversation.clear()


# ============================================================
# PERSONALITY
# ============================================================

MODES = {
    "friendly":
        "Warm, natural and conversational.",

    "professional":
        "Clear, structured and professional.",

    "concise":
        "Direct and brief.",

    "teacher":
        "Patient and educational.",

    "technical":
        "Precise, technical and structured.",

    "creative":
        "Imaginative and expressive.",

    "formal":
        "Polite and measured.",

    "energetic":
        "Upbeat and enthusiastic.",
}


class PersonalityEngine:

    @staticmethod
    def set_mode(mode: str):

        mode = mode.lower().strip()

        if mode not in MODES:
            return False

        with state_lock:
            settings["mode"] = mode

        save_data()

        return True

    @staticmethod
    def set_length(length: str):

        if length not in (
            "short",
            "normal",
            "long",
        ):
            return False

        with state_lock:
            settings["response_length"] = length

        save_data()

        return True


# ============================================================
# SAFE CALCULATOR
# ============================================================

class Calculator:

    ALLOWED_NODES = (
        ast.Expression,
        ast.Constant,
        ast.UnaryOp,
        ast.BinOp,
        ast.Add,
        ast.Sub,
        ast.Mult,
        ast.Div,
        ast.Mod,
        ast.Pow,
        ast.USub,
        ast.UAdd,
        ast.FloorDiv,
    )

    @staticmethod
    def calculate(
        expression: str,
    ):

        expression = expression.strip()

        if len(expression) > 200:
            return None

        try:

            tree = ast.parse(
                expression,
                mode="eval",
            )

            for node in ast.walk(tree):

                if not isinstance(
                    node,
                    Calculator.ALLOWED_NODES,
                ):
                    return None

            def evaluate(node):

                if isinstance(
                    node,
                    ast.Expression,
                ):
                    return evaluate(
                        node.body
                    )

                if isinstance(
                    node,
                    ast.Constant,
                ):

                    if isinstance(
                        node.value,
                        bool,
                    ):
                        raise ValueError()

                    if isinstance(
                        node.value,
                        (int, float),
                    ):

                        if not math.isfinite(
                            float(node.value)
                        ):
                            raise ValueError()

                        if abs(
                            node.value
                        ) > 10**100:
                            raise ValueError()

                        return node.value

                    raise ValueError()

                if isinstance(
                    node,
                    ast.UnaryOp,
                ):

                    value = evaluate(
                        node.operand
                    )

                    if isinstance(
                        node.op,
                        ast.USub,
                    ):
                        return -value

                    if isinstance(
                        node.op,
                        ast.UAdd,
                    ):
                        return value

                    raise ValueError()

                if isinstance(
                    node,
                    ast.BinOp,
                ):

                    left = evaluate(
                        node.left
                    )

                    right = evaluate(
                        node.right
                    )

                    if isinstance(
                        node.op,
                        ast.Add,
                    ):
                        result = left + right

                    elif isinstance(
                        node.op,
                        ast.Sub,
                    ):
                        result = left - right

                    elif isinstance(
                        node.op,
                        ast.Mult,
                    ):
                        result = left * right

                    elif isinstance(
                        node.op,
                        ast.Div,
                    ):

                        if right == 0:
                            raise ValueError()

                        result = left / right

                    elif isinstance(
                        node.op,
                        ast.Mod,
                    ):

                        if right == 0:
                            raise ValueError()

                        result = left % right

                    elif isinstance(
                        node.op,
                        ast.FloorDiv,
                    ):

                        if right == 0:
                            raise ValueError()

                        result = left // right

                    elif isinstance(
                        node.op,
                        ast.Pow,
                    ):

                        if abs(right) > 100:
                            raise ValueError()

                        result = left ** right

                    else:
                        raise ValueError()

                    if (
                        isinstance(
                            result,
                            (int, float),
                        )
                        and abs(result) > 10**100
                    ):
                        raise ValueError()

                    return result

                raise ValueError()

            result = evaluate(tree)

            if isinstance(result, float):

                if not math.isfinite(result):
                    return None

                result = round(
                    result,
                    10,
                )

            return result

        except Exception:
            return None


# ============================================================
# INTENT ENGINE
# ============================================================

class IntentEngine:

    @staticmethod
    def detect(message: str):

        text = message.lower().strip()

        rules = [
            (
                "greeting",
                [
                    r"^(hi|hello|hey|hiya)\b",
                ],
            ),

            (
                "goodbye",
                [
                    r"\bgoodbye\b",
                    r"\bsee you\b",
                    r"\bbye\b",
                ],
            ),

            (
                "identity",
                [
                    "who are you",
                    "what are you",
                ],
            ),

            (
                "capabilities",
                [
                    "what can you do",
                    "what are your abilities",
                    "your capabilities",
                ],
            ),

            (
                "how_are_you",
                [
                    "how are you",
                    "how're you",
                    "how are u",
                ],
            ),

            (
                "thanks",
                [
                    "thank you",
                    "thanks",
                    "cheers",
                ],
            ),

            (
                "recall",
                [
                    "what do you remember",
                    "show my memories",
                    "what have i told you",
                    "do you remember",
                ],
            ),

            (
                "forget_all",
                [
                    "forget everything",
                    "clear memory",
                    "clear memories",
                ],
            ),

            (
                "project",
                [
                    "what is my project called",
                    "what's my project called",
                    "what is the name of my project",
                    "what is my project name",
                ],
            ),

            (
                "slogan",
                [
                    "what is your slogan",
                    "what's your slogan",
                ],
            ),

            (
                "year",
                [
                    "what year is it",
                    "current year",
                ],
            ),

            (
                "date",
                [
                    "what date is it",
                    "today's date",
                    "what day is it",
                ],
            ),

            (
                "time",
                [
                    "what time is it",
                    "current time",
                ],
            ),

            (
                "version",
                [
                    "what version are you",
                    "your version",
                ],
            ),

            (
                "current_mode",
                [
                    "what mode are you in",
                    "which mode are you in",
                ],
            ),
        ]

        for intent, patterns in rules:

            for pattern in patterns:

                if pattern.startswith("^"):

                    if re.search(
                        pattern,
                        text,
                    ):
                        return intent

                elif pattern in text:
                    return intent

        if text.startswith("remember "):
            return "remember"

        if text.startswith("forget "):
            return "forget_one"

        if text.startswith("find memory "):
            return "memory_search"

        if text.startswith("calculate "):
            return "calculate"

        if text.startswith("calc "):
            return "calculate"

        if text.startswith("set mode "):
            return "mode"

        if (
            text.startswith("use ")
            and " mode" in text
        ):
            return "mode"

        if text.startswith(
            "set response length "
        ):
            return "response_length"

        if text.startswith(
            "teach nexora "
        ):
            return "knowledge_add"

        if text.startswith(
            "search knowledge "
        ):
            return "knowledge_search"

        if text.startswith(
            "clear conversation"
        ):
            return "clear_conversation"

        if text.startswith(
            "my name is "
        ):
            return "set_name"

        if "what is my name" in text:
            return "get_name"

        if text.startswith("explain "):
            return "explain"

        if text.startswith("define "):
            return "define"

        if text.startswith("compare "):
            return "compare"

        if "?" in text:
            return "question"

        return "conversation"


# ============================================================
# LOCAL REASONING ENGINE
# ============================================================

class ReasoningEngine:

    @staticmethod
    def retrieve(
        query: str,
    ):

        memories_found = MemoryEngine.search(
            query,
            limit=5,
        )

        knowledge_found = KnowledgeEngine.search(
            query,
            limit=5,
        )

        context = ContextEngine.recent(
            limit=8,
        )

        return {
            "memories": memories_found,
            "knowledge": knowledge_found,
            "context": context,
        }

    @staticmethod
    def rank_knowledge(
        query: str,
        results,
    ):

        ranked = []

        for score, key, value in results:

            # Extra bonus for exact subject matches.
            bonus = 0

            if normalise_text(
                query
            ) == normalise_text(key):
                bonus += 10

            ranked.append(
                (
                    score + bonus,
                    key,
                    value,
                )
            )

        ranked.sort(
            key=lambda item: item[0],
            reverse=True,
        )

        return ranked

    @staticmethod
    def reason(
        query: str,
    ):

        retrieved = ReasoningEngine.retrieve(
            query
        )

        knowledge_results = (
            ReasoningEngine.rank_knowledge(
                query,
                retrieved["knowledge"],
            )
        )

        # Strongest knowledge result.
        if knowledge_results:

            best_score, key, value = (
                knowledge_results[0]
            )

            if best_score >= 2:
                return {
                    "type": "knowledge",
                    "answer": value,
                    "confidence": min(
                        best_score / 10,
                        1.0,
                    ),
                    "source": key,
                }

        # Relevant memory.
        if retrieved["memories"]:

            return {
                "type": "memory",
                "answer": (
                    "I found something relevant "
                    "in my memory:\n\n"
                    + "\n".join(
                        "• " + item
                        for item
                        in retrieved["memories"]
                    )
                ),
                "confidence": 0.65,
                "source": "memory",
            }

        # Try calculator automatically.
        possible_math = re.sub(
            r"[^0-9+\-*/().% ]",
            "",
            query,
        ).strip()

        if (
            possible_math
            and any(
                operator in possible_math
                for operator in "+-*/%"
            )
            and len(possible_math) <= 100
        ):

            result = Calculator.calculate(
                possible_math
            )

            if result is not None:

                return {
                    "type": "calculation",
                    "answer":
                        f"The answer is {result}.",
                    "confidence": 0.99,
                    "source": "calculator",
                }

        return {
            "type": "unknown",
            "answer": None,
            "confidence": 0.0,
            "source": None,
        }


# ============================================================
# LOCAL BRAIN
# ============================================================

class LocalBrain:

    @staticmethod
    def identity():

        return (
            "I'm Nexora, a local AI assistant built around "
            "memory, knowledge retrieval, context, intent "
            "classification, reasoning tools, calculation "
            "and security systems. I'm designed to operate "
            "without an external language model."
        )

    @staticmethod
    def capabilities():

        return (
            "I can:\n\n"
            "• Remember information\n"
            "• Search and rank memories\n"
            "• Store a local knowledge base\n"
            "• Retrieve relevant knowledge\n"
            "• Maintain conversation context\n"
            "• Perform safe mathematical calculations\n"
            "• Detect user intent\n"
            "• Reason over locally stored information\n"
            "• Switch personality modes\n"
            "• Detect secrets\n"
            "• Apply safety filtering\n"
            "• Authenticate API requests\n"
            "• Rate-limit clients\n"
            "• Record security events\n"
            "• Enter lockdown/emergency mode\n"
            "• Expose a health endpoint\n"
            "• Provide a security dashboard\n\n"
            "I'm still fundamentally a local rule-based "
            "system rather than a trained neural language "
            "model, so I won't invent knowledge that I don't "
            "actually have."
        )

    @staticmethod
    def project_name():

        patterns = [
            r"project\s+(?:is\s+)?called\s+(.+)",
            r"project\s+name\s+(?:is\s+)?(.+)",
            r"my\s+project\s+is\s+(.+)",
        ]

        for memory in MemoryEngine.all():

            for pattern in patterns:

                match = re.search(
                    pattern,
                    memory,
                    re.IGNORECASE,
                )

                if match:

                    return (
                        match.group(1)
                        .strip()
                        .rstrip(".")
                    )

        results = KnowledgeEngine.search(
            "project name"
        )

        for _, key, value in results:

            if (
                "project" in key.lower()
                or "project" in value.lower()
            ):
                return value.strip().rstrip(".")

        return None

    @staticmethod
    def answer(
        message: str,
        intent: str,
    ):

        text = message.strip()

        if intent == "greeting":
            name = settings.get(
                "user_name"
            )

            if name:
                return (
                    f"Hey {name}! 👋 "
                    "I'm Nexora. What are we "
                    "working on?"
                )

            return (
                "Hey! 👋 I'm Nexora. "
                "What are we working on?"
            )

        if intent == "goodbye":
            return "See you later! 👋"

        if intent == "identity":
            return LocalBrain.identity()

        if intent == "capabilities":
            return LocalBrain.capabilities()

        if intent == "how_are_you":
            return (
                "I'm running normally and ready to help. "
                "What shall we work on?"
            )

        if intent == "thanks":
            return "You're welcome! 😊"

        if intent == "slogan":
            return f"My slogan is: {SLOGAN}"

        if intent == "version":
            return f"I'm running Nexora v{VERSION}."

        if intent == "year":
            return (
                f"The current year is "
                f"{datetime.now().year}."
            )

        if intent == "date":
            return (
                "Today is "
                + datetime.now().strftime(
                    "%A, %d %B %Y"
                )
                + "."
            )

        if intent == "time":
            return (
                "The server's current local time is "
                + datetime.now().strftime(
                    "%H:%M:%S"
                )
                + "."
            )

        if intent == "project":

            project = LocalBrain.project_name()

            if project:

                return (
                    f"Your project is called "
                    f"{project}."
                )

            return (
                "I don't have your project name "
                "saved yet. Try saying: "
                "\"remember that my project is called "
                "Nexora\"."
            )

        if intent == "remember":

            memory = re.sub(
                r"^remember\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            if memory.lower().startswith(
                "that "
            ):
                memory = memory[5:].strip()

            try:

                MemoryEngine.save(
                    memory
                )

                return (
                    "Got it. I've saved that "
                    "to my memory."
                )

            except SafetyFailure:

                return (
                    "I couldn't safely store that."
                )

        if intent == "recall":

            saved = MemoryEngine.all()

            if not saved:
                return (
                    "I don't have any saved "
                    "memories yet."
                )

            return (
                "Here's what I remember:\n\n"
                + "\n".join(
                    f"{i}. {item}"
                    for i, item
                    in enumerate(
                        saved,
                        1,
                    )
                )
            )

        if intent == "forget_all":

            MemoryEngine.clear()

            return (
                "Done. I've cleared my "
                "saved memories."
            )

        if intent == "forget_one":

            target = re.sub(
                r"^forget\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            if MemoryEngine.delete_matching(
                target
            ):

                return (
                    "Done. I've forgotten the "
                    "matching memory."
                )

            return (
                "I couldn't find a matching memory."
            )

        if intent == "memory_search":

            query = re.sub(
                r"^find memory\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            results = MemoryEngine.search(
                query,
                limit=8,
            )

            if not results:

                return (
                    "I couldn't find a matching "
                    "memory."
                )

            return (
                "Matching memories:\n\n"
                + "\n".join(
                    "• " + item
                    for item in results
                )
            )

        if intent == "calculate":

            expression = re.sub(
                r"^(calculate|calc)\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            result = Calculator.calculate(
                expression
            )

            if result is None:
                return (
                    "I couldn't safely calculate that."
                )

            return f"The answer is {result}."

        if intent == "mode":

            match = re.search(
                r"(?:use|set mode)\s+"
                r"([a-zA-Z]+)",
                text,
                re.IGNORECASE,
            )

            if not match:

                return (
                    "Available modes: "
                    + ", ".join(
                        MODES.keys()
                    )
                )

            mode = match.group(1).lower()

            if PersonalityEngine.set_mode(
                mode
            ):

                return (
                    f"Speaking mode changed "
                    f"to {mode}."
                )

            return (
                "I don't know that mode.\n\n"
                "Available modes: "
                + ", ".join(
                    MODES.keys()
                )
            )

        if intent == "current_mode":

            return (
                "I'm currently using "
                + settings.get(
                    "mode",
                    "friendly",
                )
                + " mode."
            )

        if intent == "response_length":

            match = re.search(
                r"set response length\s+"
                r"(short|normal|long)",
                text,
                re.IGNORECASE,
            )

            if not match:

                return (
                    "Available response lengths: "
                    "short, normal, long."
                )

            length = match.group(1).lower()

            if PersonalityEngine.set_length(
                length
            ):

                return (
                    f"Response length changed "
                    f"to {length}."
                )

            return (
                "I couldn't change the "
                "response length."
            )

        if intent == "set_name":

            name = re.sub(
                r"^my name is\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            if not name or len(name) > 100:

                return (
                    "I couldn't safely save "
                    "that name."
                )

            if contains_secret(name):

                return (
                    "I can't store secrets "
                    "as your name."
                )

            with state_lock:
                settings["user_name"] = name

            save_data()

            return f"Nice to meet you, {name}."

        if intent == "get_name":

            name = settings.get(
                "user_name"
            )

            if name:
                return (
                    f"Your saved name is {name}."
                )

            return (
                "I don't have your name "
                "saved yet."
            )

        if intent == "knowledge_add":

            content = re.sub(
                r"^teach nexora\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            if "=" not in content:

                return (
                    "Use this format:\n"
                    "teach nexora topic = information"
                )

            key, value = content.split(
                "=",
                1,
            )

            if KnowledgeEngine.add(
                key,
                value,
            ):

                return (
                    "I've added that to my "
                    "knowledge base."
                )

            return (
                "I couldn't safely add that."
            )

        if intent == "knowledge_search":

            query = re.sub(
                r"^search knowledge\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            results = KnowledgeEngine.search(
                query
            )

            if not results:

                return (
                    "I couldn't find anything "
                    "matching that."
                )

            return "\n".join(
                f"• {key}: {value}"
                for _, key, value
                in results
            )

        if intent == "clear_conversation":

            ContextEngine.clear()

            return (
                "I've cleared the current "
                "conversation context."
            )

        if intent == "explain":

            topic = re.sub(
                r"^explain\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            return LocalBrain.explain(
                topic
            )

        if intent == "define":

            topic = re.sub(
                r"^define\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            results = KnowledgeEngine.search(
                topic
            )

            if results:
                return results[0][2]

            return (
                f"I don't have a reliable local "
                f"definition of '{topic}' yet."
            )

        if intent == "compare":

            topic = re.sub(
                r"^compare\s+",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()

            parts = re.split(
                r"\s+(?:vs\.?|versus|and)\s+",
                topic,
                maxsplit=1,
                flags=re.IGNORECASE,
            )

            if len(parts) == 2:

                first = parts[0].strip()
                second = parts[1].strip()

                first_info = (
                    KnowledgeEngine.search(
                        first,
                        limit=1,
                    )
                )

                second_info = (
                    KnowledgeEngine.search(
                        second,
                        limit=1,
                    )
                )

                output = [
                    "Comparison:",
                    "",
                    first + ":",
                ]

                if first_info:
                    output.append(
                        first_info[0][2]
                    )
                else:
                    output.append(
                        "I don't have reliable "
                        "local information about this."
                    )

                output.extend([
                    "",
                    second + ":",
                ])

                if second_info:
                    output.append(
                        second_info[0][2]
                    )
                else:
                    output.append(
                        "I don't have reliable "
                        "local information about this."
                    )

                return "\n".join(output)

            return (
                "Tell me two things to compare, "
                "for example: compare cats vs dogs."
            )

        # ====================================================
        # ADVANCED LOCAL REASONING
        # ====================================================

        reasoning = ReasoningEngine.reason(
            text
        )

        if reasoning["answer"]:

            return reasoning["answer"]

        match = re.match(
            r"what\s+is\s+(.+?)[?]?$",
            text,
            re.IGNORECASE,
        )

        if match:

            subject = match.group(
                1
            ).strip()

            results = KnowledgeEngine.search(
                subject
            )

            if results:

                return results[0][2]

            return (
                f"I don't have enough reliable "
                f"local knowledge about "
                f"'{subject}' to give you a "
                f"trustworthy answer."
            )

        return (
            "I understand what you're saying. "
            "I don't have a trained external language "
            "model, so I won't pretend to know something "
            "I don't. You can teach me information with "
            "'teach Nexora topic = information', or "
            "save something with 'remember that ...'."
        )

    @staticmethod
    def explain(topic: str):

        if not topic:

            return (
                "Tell me what you'd like me "
                "to explain."
            )

        results = KnowledgeEngine.search(
            topic
        )

        if results:

            return (
                "Here's what I know:\n\n"
                + results[0][2]
            )

        return (
            f"I don't have enough reliable local "
            f"knowledge to explain '{topic}' yet.\n\n"
            f"You can teach me using:\n"
            f"teach Nexora {topic} = information"
        )


# ============================================================
# BRAIN
# ============================================================

class NexoraBrain:

    @staticmethod
    def generate(
        message: str,
    ):

        intent = IntentEngine.detect(
            message
        )

        return LocalBrain.answer(
            message,
            intent,
        )


# ============================================================
# OUTPUT SECURITY
# ============================================================

def validate_output(
    response: str,
):

    if not isinstance(
        response,
        str,
    ):
        raise SafetyFailure()

    response = response.strip()

    if not response:
        raise SafetyFailure()

    if len(response) > MAX_OUTPUT:

        security_event(
            "oversized_output",
            "ERROR",
        )

        raise SafetyFailure()

    allowed, category = safety_check(
        response
    )

    if not allowed:

        security_event(
            "unsafe_output",
            "CRITICAL",
        )

        raise SafetyFailure()

    if contains_secret(response):

        security_event(
            "secret_output",
            "CRITICAL",
        )

        raise SafetyFailure()

    return response


# ============================================================
# RESPONSE STYLE
# ============================================================

def apply_style(
    response: str,
):

    length = settings.get(
        "response_length",
        "normal",
    )

    mode = settings.get(
        "mode",
        "friendly",
    )

    if (
        length == "short"
        or mode == "concise"
    ):

        lines = response.splitlines()

        if len(lines) > 6:

            response = "\n".join(
                lines[:6]
            )

    if not settings.get(
        "emoji",
        True,
    ):

        response = re.sub(
            r"[\U0001F300-\U0001FAFF]",
            "",
            response,
        )

    return response.strip()


# ============================================================
# MAIN PROCESSING PIPELINE
# ============================================================

def process(
    message: str,
    client_id: str,
):

    try:

        with state_lock:
            metrics["requests"] += 1

        message = validate_input(
            message
        )

        rate_limit(
            client_id
        )

        if is_locked():

            with state_lock:
                metrics["blocked_requests"] += 1

            return (
                "Nexora is currently in "
                "security lockdown."
            )

        if contains_secret(
            message
        ):

            with state_lock:
                metrics["blocked_requests"] += 1

            security_event(
                "secret_input_blocked",
                "WARN",
            )

            return (
                "For your privacy, please don't "
                "send passwords, API keys, tokens "
                "or other secrets in chat."
            )

        allowed, category = safety_check(
            message
        )

        if not allowed:

            with state_lock:
                metrics["blocked_requests"] += 1

            security_event(
                "unsafe_request",
                "WARN",
            )

            return safety_response(
                category
            )

        ContextEngine.add(
            "user",
            message,
        )

        response = NexoraBrain.generate(
            message
        )

        response = apply_style(
            response
        )

        response = validate_output(
            response
        )

        ContextEngine.add(
            "assistant",
            response,
        )

        with state_lock:
            metrics["successful_requests"] += 1

        return response

    except (
        SafetyFailure,
        AuthenticationFailure,
    ):

        with state_lock:
            metrics["failed_requests"] += 1

        security_event(
            "request_failed_closed",
            "WARN",
        )

        return (
            "Nexora couldn't safely "
            "complete that request."
        )

    except Exception:

        with state_lock:
            metrics["failed_requests"] += 1

        security_event(
            "unexpected_failure",
            "CRITICAL",
        )

        return (
            "Nexora encountered a problem "
            "and stopped safely."
        )


# ============================================================
# SECURITY DASHBOARD
# ============================================================

def security_snapshot():

    with state_lock:

        uptime = (
            time.time()
            - metrics["started_at"]
        )

        return {
            "application": APP_NAME,
            "version": VERSION,
            "security_state": security_state,
            "api_authentication": bool(
                API_KEY
            ),
            "api_key_configured": bool(
                API_KEY
            ),
            "rate_limit": {
                "requests": RATE_LIMIT,
                "window_seconds": RATE_WINDOW,
            },
            "metrics": {
                "requests":
                    metrics["requests"],
                "successful_requests":
                    metrics[
                        "successful_requests"
                    ],
                "failed_requests":
                    metrics[
                        "failed_requests"
                    ],
                "blocked_requests":
                    metrics[
                        "blocked_requests"
                    ],
                "authenticated_requests":
                    metrics[
                        "authenticated_requests"
                    ],
            },
            "storage": {
                "memories":
                    len(memories),
                "knowledge_entries":
                    len(knowledge),
            },
            "security_events":
                len(security_events),
            "uptime_seconds":
                round(uptime, 2),
            "features": {
                "secret_detection": True,
                "safety_filter": True,
                "memory_injection_detection": True,
                "safe_calculator": True,
                "rate_limiting": True,
                "atomic_storage": True,
                "api_authentication": bool(API_KEY),
            },
        }


# ============================================================
# HTML
# ============================================================

HTML = """
<!DOCTYPE html>
<html lang="en">

<head>

<meta charset="UTF-8">

<meta
    name="viewport"
    content="width=device-width, initial-scale=1.0"
>

<title>Nexora v10</title>

<style>

* {
    box-sizing: border-box;
}

body {
    margin: 0;
    min-height: 100vh;

    background:
        radial-gradient(
            circle at 10% 10%,
            #36006b,
            transparent 30%
        ),
        radial-gradient(
            circle at 90% 90%,
            #004b59,
            transparent 30%
        ),
        #050008;

    color: white;

    font-family:
        Inter,
        Arial,
        sans-serif;
}

.container {
    width: min(1100px, 94%);
    margin: 28px auto;
}

.header {
    text-align: center;
    margin-bottom: 20px;
}

.logo {
    font-size: 62px;
    font-weight: 900;

    background:
        linear-gradient(
            90deg,
            white,
            #6fffff,
            #d000ff
        );

    color: transparent;
    background-clip: text;
    -webkit-background-clip: text;
}

.slogan {
    color: #67ffff;

    font-size: 12px;
    letter-spacing: 6px;
}

.status {
    color: #888;

    font-size: 12px;
    margin-top: 8px;
}

.security-bar {
    display: flex;
    justify-content: space-between;
    align-items: center;

    margin-bottom: 12px;
    padding: 10px 14px;

    border: 1px solid
        rgba(103, 255, 255, .15);

    border-radius: 12px;

    background:
        rgba(10, 10, 20, .75);

    font-size: 12px;
}

.security-dot {
    display: inline-block;

    width: 8px;
    height: 8px;

    margin-right: 7px;

    border-radius: 50%;

    background: #55ff99;

    box-shadow:
        0 0 10px #55ff99;
}

.chat {
    height: 62vh;
    min-height: 430px;

    overflow-y: auto;

    padding: 22px;

    border:
        1px solid
        rgba(180, 0, 255, .4);

    border-radius: 20px;

    background:
        rgba(5, 4, 12, .9);

    box-shadow:
        0 0 50px
        rgba(150, 0, 255, .12);
}

.message {
    max-width: 82%;

    padding: 14px 17px;
    margin-bottom: 16px;

    border-radius: 16px;

    white-space: pre-wrap;

    line-height: 1.55;
}

.nexora {
    border-left:
        3px solid #67ffff;

    background:
        rgba(25, 25, 45, .85);
}

.user {
    margin-left: auto;

    background:
        linear-gradient(
            135deg,
            #6500ff,
            #c000ff
        );
}

.sender {
    display: block;

    margin-bottom: 6px;

    color: #67ffff;

    font-size: 10px;

    font-weight: bold;

    letter-spacing: 2px;
}

.composer {
    display: flex;

    gap: 10px;

    margin-top: 15px;
}

input {
    flex: 1;

    padding: 16px;

    border:
        1px solid #7500ff;

    border-radius: 13px;

    background: #110819;

    color: white;

    font-size: 15px;

    outline: none;
}

button {
    border: 0;

    border-radius: 13px;

    padding: 0 25px;

    color: white;

    font-weight: bold;

    background:
        linear-gradient(
            135deg,
            #7000ff,
            #c000ff
        );

    cursor: pointer;
}

button:hover {
    opacity: .9;
}

.panel {
    margin-top: 15px;

    padding: 15px;

    border:
        1px solid
        rgba(180, 0, 255, .25);

    border-radius: 15px;

    background:
        rgba(10, 8, 20, .8);

    display: none;
}

.panel input {
    width: 100%;
    margin-bottom: 10px;
}

.panel button {
    padding: 10px 15px;
}

.security-data {
    white-space: pre-wrap;

    color: #b8ffff;

    font-family: monospace;

    font-size: 12px;
}

</style>

</head>

<body>

<div class="container">

<div class="header">

<div class="logo">
Nexora
</div>

<div class="slogan">
INTELLIGENCE. SECURED.
</div>

<div class="status">
v10.0 • Local Reasoning • Memory • Context • Security
</div>

</div>

<div class="security-bar">

<div>
<span class="security-dot"></span>
SECURITY SYSTEM ACTIVE
</div>

<button
    type="button"
    onclick="toggleSecurity()"
>
Security
</button>

</div>

<div
    class="panel"
    id="securityPanel"
>

<input
    id="apiKey"
    type="password"
    placeholder="Enter Nexora API key"
    autocomplete="off"
>

<button
    type="button"
    onclick="saveKey()"
>
Save API key
</button>

<button
    type="button"
    onclick="loadSecurity()"
>
Refresh security
</button>

<div
    class="security-data"
    id="securityData"
>
Security information unavailable.
</div>

</div>

<div
    class="chat"
    id="chat"
>

<div class="message nexora">

<span class="sender">
NEXORA
</span>

Hey! I'm Nexora v10. 👋

I'm running locally without an
external language model.

What are we working on?

</div>

</div>

<form
    class="composer"
    onsubmit="sendMessage(event)"
>

<input
    id="message"
    maxlength="5000"
    autocomplete="off"
    placeholder="Talk to Nexora..."
>

<button>
Send
</button>

</form>

</div>

<script>

function getKey() {
    return sessionStorage.getItem(
        "nexora_api_key"
    ) || "";
}


function saveKey() {

    const input =
        document.getElementById(
            "apiKey"
        );

    const key =
        input.value.trim();

    if (!key) {
        return;
    }

    sessionStorage.setItem(
        "nexora_api_key",
        key
    );

    input.value = "";

    loadSecurity();
}


function toggleSecurity() {

    const panel =
        document.getElementById(
            "securityPanel"
        );

    if (
        panel.style.display === "block"
    ) {

        panel.style.display = "none";

    } else {

        panel.style.display = "block";

        loadSecurity();
    }
}


function addMessage(
    sender,
    text,
    type
) {

    const chat =
        document.getElementById(
            "chat"
        );

    const message =
        document.createElement(
            "div"
        );

    message.className =
        "message " + type;

    const senderElement =
        document.createElement(
            "span"
        );

    senderElement.className =
        "sender";

    senderElement.textContent =
        sender;

    message.appendChild(
        senderElement
    );

    message.appendChild(
        document.createTextNode(text)
    );

    chat.appendChild(
        message
    );

    chat.scrollTop =
        chat.scrollHeight;
}


async function sendMessage(event) {

    event.preventDefault();

    const input =
        document.getElementById(
            "message"
        );

    const text =
        input.value.trim();

    if (!text) {
        return;
    }

    addMessage(
        "YOU",
        text,
        "user"
    );

    input.value = "";

    try {

        const response =
            await fetch(
                "/api/chat",
                {
                    method: "POST",

                    headers: {
                        "Content-Type":
                            "application/json",

                        "Authorization":
                            "Bearer " + getKey()
                    },

                    body: JSON.stringify({
                        message: text
                    })
                }
            );

        const data =
            await response.json();

        addMessage(
            "NEXORA",
            data.reply ||
            data.error ||
            "Nexora couldn't safely answer.",
            "nexora"
        );

    } catch (error) {

        addMessage(
            "NEXORA",
            "I couldn't connect to the Nexora server.",
            "nexora"
        );
    }
}


async function loadSecurity() {

    const output =
        document.getElementById(
            "securityData"
        );

    try {

        const response =
            await fetch(
                "/api/security",
                {
                    headers: {
                        "Authorization":
                            "Bearer " + getKey()
                    }
                }
            );

        const data =
            await response.json();

        output.textContent =
            JSON.stringify(
                data,
                null,
                2
            );

    } catch (error) {

        output.textContent =
            "Unable to load security information.";
    }
}

</script>

</body>

</html>
"""


# ============================================================
# HTTP SERVER
# ============================================================

class NexoraServer(
    BaseHTTPRequestHandler
):

    def json_response(
        self,
        data,
        status=200,
    ):

        body = json.dumps(
            data,
            ensure_ascii=False,
        ).encode("utf-8")

        self.send_response(
            status
        )

        self.send_header(
            "Content-Type",
            "application/json; charset=utf-8",
        )

        self.send_header(
            "Cache-Control",
            "no-store",
        )

        self.send_header(
            "X-Content-Type-Options",
            "nosniff",
        )

        self.send_header(
            "Content-Length",
            str(len(body)),
        )

        self.end_headers()

        self.wfile.write(
            body
        )

    def do_GET(self):

        path = urlparse(
            self.path
        ).path

        if path == "/":

            body = HTML.encode(
                "utf-8"
            )

            self.send_response(
                200
            )

            self.send_header(
                "Content-Type",
                "text/html; charset=utf-8",
            )

            self.send_header(
                "X-Content-Type-Options",
                "nosniff",
            )

            self.send_header(
                "Content-Length",
                str(len(body)),
            )

            self.end_headers()

            self.wfile.write(
                body
            )

            return

        if path == "/health":

            self.json_response({
                "status": "ok",
                "name": APP_NAME,
                "version": VERSION,
                "slogan": SLOGAN,
                "local_ai": True,
                "external_model": False,
                "memory": True,
                "knowledge": True,
                "context": True,
                "calculator": True,
                "safety": True,
                "privacy": True,
                "api_authentication":
                    bool(API_KEY),
                "fail_closed": True,
                "lockdown": is_locked(),
                "mode": settings.get(
                    "mode",
                    "friendly",
                ),
            })

            return

        if path == "/api/security":

            try:

                authenticate_request(
                    self
                )

                self.json_response(
                    security_snapshot()
                )

            except AuthenticationFailure:

                self.json_response(
                    {
                        "error":
                            "Authentication required."
                    },
                    401,
                )

            return

        self.json_response(
            {
                "error":
                    "Not found"
            },
            404,
        )

    def do_POST(self):

        path = urlparse(
            self.path
        ).path

        if path != "/api/chat":

            self.json_response(
                {
                    "error":
                        "Not found"
                },
                404,
            )

            return

        try:

            authenticate_request(
                self
            )

        except AuthenticationFailure:

            self.json_response(
                {
                    "error":
                        "Authentication required."
                },
                401,
            )

            return

        try:

            length = int(
                self.headers.get(
                    "Content-Length",
                    "0",
                )
            )

        except ValueError:

            self.json_response(
                {
                    "error":
                        "Invalid request"
                },
                400,
            )

            return

        if (
            length <= 0
            or length > MAX_REQUEST_BYTES
        ):

            self.json_response(
                {
                    "error":
                        "Request too large"
                },
                413,
            )

            return

        try:

            raw = self.rfile.read(
                length
            )

            data = json.loads(
                raw.decode("utf-8")
            )

            if not isinstance(
                data,
                dict,
            ):

                self.json_response(
                    {
                        "error":
                            "Invalid JSON object"
                    },
                    400,
                )

                return

            message = data.get(
                "message"
            )

            if not isinstance(
                message,
                str,
            ):

                self.json_response(
                    {
                        "error":
                            "Invalid message"
                    },
                    400,
                )

                return

            client_id = (
                self.client_address[0]
            )

            reply = process(
                message,
                client_id,
            )

            self.json_response({
                "reply": reply
            })

        except json.JSONDecodeError:

            self.json_response(
                {
                    "error":
                        "Invalid JSON"
                },
                400,
            )

        except Exception:

            security_event(
                "http_failure",
                "ERROR",
            )

            self.json_response(
                {
                    "error":
                        "Request could not "
                        "be safely processed."
                },
                500,
            )

    def log_message(
        self,
        format_string,
        *args,
    ):
        # Don't write chat content
        # into Render logs.
        return


# ============================================================
# STARTUP
# ============================================================

def startup():

    load_data()

    print("=" * 60)
    print("NEXORA v10")
    print("Intelligence. Secured.")
    print("=" * 60)

    print(
        f"Version: {VERSION}"
    )

    print(
        f"Host: {HOST}"
    )

    print(
        f"Port: {PORT}"
    )

    print(
        "Local reasoning: ENABLED"
    )

    print(
        "Persistent memory: ENABLED"
    )

    print(
        "Knowledge engine: ENABLED"
    )

    print(
        "Context engine: ENABLED"
    )

    print(
        "Intent engine: ENABLED"
    )

    print(
        "Reasoning engine: ENABLED"
    )

    print(
        "Safe calculator: ENABLED"
    )

    print(
        "Secret detection: ENABLED"
    )

    print(
        "Safety engine: ENABLED"
    )

    print(
        "Rate limiting: ENABLED"
    )

    print(
        "API authentication:",
        "ENABLED" if API_KEY else "DISABLED",
    )

    if not API_KEY:

        print()
        print(
            "WARNING:"
        )
        print(
            "NEXORA_API_KEY is not configured."
        )
        print(
            "The API is therefore locked until "
            "you configure a key."
        )
        print()

    print(
        "External model: DISABLED"
    )

    print("=" * 60)

    server = ThreadingHTTPServer(
        (HOST, PORT),
        NexoraServer,
    )

    try:

        server.serve_forever()

    except KeyboardInterrupt:

        print(
            "Nexora shutting down."
        )

    finally:

        server.server_close()


# ============================================================
# COMMAND LINE
# ============================================================

def main():

    if (
        len(os.sys.argv) > 1
        and os.sys.argv[1]
        == "--generate-key"
    ):

        print(
            generate_api_key()
        )

        return

    startup()


# ============================================================
# RUN
# ============================================================

if __name__ == "__main__":
    main()
    


